...
def write_first_line_to_file(file_contents, output_filename):
    ...
    with open(output_filename, 'w') as destination_file:
        StoredContent = file_contents.split("\n")
        destination_file.write(StoredContent[0])
...
def read_even_numbered_lines(file_name):
    ...
    with open(file_name) as read_file:
        count = 1
        for new_data_Content in read_file.readlines():
            if (count % 2 == 0):
                new_even_list.append(new_data_Content)
            count += 1
    # printing list as new line
